...
func (f *fakeReporter) snapshot() (bool, bool, int) {
	f.mu.Lock()
	defer f.mu.Unlock()
	return f.prCreated, f.reviewCreated, len(f.issueComments)
}
...
func TestManagerRunJob_CreatesPRAndReview(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		if ok && (job.Status == JobStatusSucceeded || job.Status == JobStatusFailed) {
			...
			prCreated, reviewCreated, issueComments := reporter.snapshot()
			if !prCreated {
