...
// esc escapes a string for safe interpolation into the HTML report. Resource
// names are DNS-constrained today, but free-text fields (AI analysis, event
// messages, security-issue descriptions, node OS/taints) are attacker- or
// LLM-influenced and must be escaped to prevent stored XSS when the report is
// served as text/html.
...
`)
	...
	sb.WriteString(fmt.Sprintf(`<p><strong>Generated By:</strong> %s</p>`, esc(report.GeneratedBy)))
	sb.WriteString(fmt.Sprintf(`<p><strong>Cluster Version:</strong> %s</p>`, esc(report.ClusterInfo.ServerVersion)))
	sb.WriteString(`</div>`)
...
<div style="color: #666; margin-top: 10px;">Overall Cluster Health Score - <strong class="status-%s">%s</strong></div>
	...
	if sections.SecurityBasic && report.SecurityScan != nil {
		...
		if len(report.SecurityScan.PodSecurityIssues) > 0 {
			...
			for i, issue := range report.SecurityScan.PodSecurityIssues {
				...
				sb.WriteString(fmt.Sprintf(`<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td class="%s">%s</td></tr>`,
					esc(issue.Namespace), esc(issue.Pod), esc(issue.Issue), esc(issue.Severity), sevClass, status))
			}
			sb.WriteString(`</table>`)
		}
		...
		if len(report.SecurityScan.RBACIssues) > 0 {
			...
			for i, issue := range report.SecurityScan.RBACIssues {
				...
				sb.WriteString(fmt.Sprintf(`<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td class="%s">%s</td></tr>`,
					esc(issue.Kind), esc(issue.Name), esc(issue.Issue), esc(issue.Severity), sevClass, status))
			}
			sb.WriteString(`</table>`)
		}
	...
	if report.AIAnalysis != "" {
		...
		sb.WriteString(fmt.Sprintf(`<div class="ai-analysis">%s</div>`, esc(report.AIAnalysis)))
	}
	...
	if sections.Nodes {
		...
		for _, node := range report.Nodes {
			...
			sb.WriteString(fmt.Sprintf(`<tr><td>%s</td><td class="%s">%s</td><td>%s</td><td>%s</td><td>%s / %s</td><td>%s / %s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>`,
				esc(node.Name), statusClass, status, esc(strings.Join(node.Roles, ", ")), esc(node.KubeletVersion), node.CPUCapacity, node.CPUAllocatable, node.MemoryCapacity, node.MemoryAllocatable, schedulable, esc(warnings), esc(taints), esc(node.InternalIP)))
		}
		sb.WriteString(`</table>`)
	}
		...
		if len(report.FinOpsAnalysis.CostOptimizations) > 0 {
			...
			for _, opt := range report.FinOpsAnalysis.CostOptimizations {
				...
				sb.WriteString(fmt.Sprintf(`<tr><td class="%s">%s</td><td>%s</td><td>%s</td><td>%s</td><td>$%.2f</td></tr>`,
					priorityClass, esc(strings.ToUpper(opt.Priority)), esc(opt.Category), esc(opt.Description), esc(opt.Impact), opt.EstimatedSaving))
			}
			sb.WriteString(`</table>`)
		}
	}
	...
	if sections.Events && len(report.Events) > 0 {
		...
		for i, event := range report.Events {
			...
			sb.WriteString(fmt.Sprintf(`<tr><td>%s</td><td>%s</td><td>%s</td><td>%d</td></tr>`,
				esc(event.Reason), esc(event.Object), esc(msg), event.Count))
		}
		sb.WriteString(`</table>`)
	}
