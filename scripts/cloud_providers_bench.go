...
var models = []struct {
	name     string
	provider string // openai, gemini, solar
	model    string
	endpoint string
}{
	// OpenAI latest models (GPT-5 series)
	{"gpt-5", "openai", "gpt-5", "https://api.openai.com/v1"},
	{"gpt-5-mini", "openai", "gpt-5-mini", "https://api.openai.com/v1"},
	{"o3-mini", "openai", "o3-mini", "https://api.openai.com/v1"},
	// Gemini 3 series
	{"gemini-3-flash", "gemini", "gemini-3-flash-preview", "https://generativelanguage.googleapis.com/v1beta/openai"},
	{"gemini-3-pro", "gemini", "gemini-3-pro-preview", "https://generativelanguage.googleapis.com/v1beta/openai"},
	// Solar for comparison
	{"solar-pro2 (high)", "solar", "solar-pro2", "https://api.upstage.ai/v1"},
}
...
func main() {
	...
	fmt.Println("  Cloud Provider AI Model Benchmark")
	fmt.Println("  OpenAI, Gemini, Solar Pro2 Comparison")
	...
	for _, model := range models {
		...
		for _, task := range benchTasks {
			start := time.Now()
			response, err := callAPI(model.provider, model.endpoint, model.model, task.prompt)
	...
	// Final summary
	fmt.Println("\n" + strings.Repeat("=", 70))
	fmt.Println("  FINAL RESULTS - Cloud Provider Comparison")
	fmt.Println(strings.Repeat("=", 70))
	fmt.Println()
	fmt.Printf("%-22s %6s %8s %6s %8s %10s\n", "Model", "Easy", "Medium", "Hard", "Total", "Avg Time")
	fmt.Println(strings.Repeat("-", 70))
	...
	for _, model := range models {
		...
		if totalTasks > 0 {
			...
			fmt.Printf("%-22s %d/%d    %d/%d     %d/%d   %5.1f%%  %8.3fs\n",
				model.name,
				stats.easy, stats.easyTotal,
				stats.medium, stats.mediumTotal,
				stats.hard, stats.hardTotal,
...
func callAPI(provider, endpoint, model, prompt string) (string, error) {
	...
	case "openai":
		apiKey = openaiAPIKey
		// o3-mini doesn't support system messages
		if model == "o3-mini" {
			reqBody = ChatRequest{
				Model: model,
				Messages: []Message{
	...
	if len(chatResp.Choices) == 0 {
		return "", fmt.Errorf("no response: %s", string(body)[:min(200, len(body))])
	}
