...
### Changed
- **Multi-DB backend support (partial)**: The `web_settings`, `model_profiles`, and `llm_usage` tables used SQLite-only DDL (`INTEGER PRIMARY KEY AUTOINCREMENT`, unquoted `key`) that fails to create on the configured PostgreSQL and MariaDB/MySQL backends. Ported their DDL to be dialect-aware, matching the existing `audit_logs` convention (`SERIAL`/`BIGINT AUTO_INCREMENT`, reserved-word quoting, `ENGINE=InnoDB`). Added a `Rebind` helper that converts `?` placeholders to PostgreSQL's `$N` form (no-op for SQLite/MySQL) with the `web_settings` upsert now using the correct `ON CONFLICT` vs `ON DUPLICATE KEY UPDATE` dialect. The SQLite path is byte-identical (covered by a new round-trip test); remaining DML placeholder rebinding across the rest of the query layer is tracked centrally in `dialect.go` for full PostgreSQL support.
